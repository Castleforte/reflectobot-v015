...
import { playExclusiveAudio, stopCurrentAudio, playRobotNameAudio, stopHoverAudio } from '../utils/audioManager';
...
function MyBotSection({ onClose, selectedRobotId, onSelectRobot, isRobotSpeaking, setIsRobotSpeaking }: MyBotSectionProps) {
  ...
  const handleMouseEnter = (robot: any) => {
    ...
    // Play robot name audio (controlled by voices toggle, no button sounds)
    if (robot.nameAudio) {
      playRobotNameAudio(robot.nameAudio);
    }
  };
