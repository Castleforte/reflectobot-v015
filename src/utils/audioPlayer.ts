...
export const playAudioFromText = async (
  ...
  try {
    // Check if voices are enabled in progress settings
    const progress = loadProgress();
    if (!progress.voicesEnabled) {
      console.log('🔊 Voice playback skipped: Voices are disabled in settings.');
      return; // Do not play voice audio if voices are disabled
    }
